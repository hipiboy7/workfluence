-- P8 (FR-901·903). 실시간 편집 저장의 멘션을 **그 멘션을 만든 사람**에게 귀속한다.
-- 설계: docs/P8_설계서_Mention.md C.2·D절. 손으로 쓴다 (보류 17).

-- ── 부른 사람을 모를 수 있다 ──────────────────────────────────────────────
--
-- 실시간 편집의 자동 저장에는 요청한 사람이 없다. 멘션을 누가 만들었는지 확실히 알 수
...
--   "gone": [["<이름>", ["<users.id>" | null, …]], …]}`
-- 멘션 자리(`@` 글자의 ID + 이름)마다 만든 사람, 어느 연결이 어느 글자를 들여왔나, 저장 전까지 사라진 이름.
-- `null`은 모름이다.
