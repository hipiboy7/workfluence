...
import { buildChatRequest, createSseParser, readChatChunk, readErrorMessage, readModelIds, type ChatMessage } from './domain/openai';
...
/** 모델 목록 응답의 상한 (D.2) */
const MODELS_BODY_MAX = 1024 * 1024;
...
export class OpenAiCompatClient implements LlmClient {
  async *stream(target: LlmTarget, messages: ChatMessage[], signal: AbortSignal): AsyncIterable<LlmChunk> {
    ...
    if (!res.ok) throw new LlmError('rejected', readErrorMessage(res.status, await readLimited(res, ERROR_BODY_MAX, false)));
    ...
    try {
      for (;;) {
        ...
        const events = r.done ? [...sse.push(text), ...sse.end()] : sse.push(text);
        for (const data of events) {
          const c = readChatChunk(data);
          // 흐름 안의 오류 — 모델 서버가 도중에 거절했거나(그 메시지) 읽을 수 없는 조각이다. 까닭은 메시지가 말한다
          if (c.error !== null) throw new LlmError('rejected', c.error);
          if (c.done) {
            yield* think.end();
            return;
          }
  ...
  async listModels(target: LlmTarget, signal: AbortSignal): Promise<string[]> {
    ...
    const body = await readLimited(res, res.ok ? MODELS_BODY_MAX : ERROR_BODY_MAX, res.ok);
    if (!res.ok) throw new LlmError('rejected', readErrorMessage(res.status, body));
    let json: unknown;
    try {
