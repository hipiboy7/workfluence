...
describe('거르기 — 남의 몫은 빼고, 이름은 서버가 정한다 (D.5)', () => {
  ...
  it('보낸 사람이 주인인 몫은 남기고 이름을 서버가 아는 표시 이름으로 바꾼다 — 색과 커서는 그대로', () => {
    const cursor = { anchor: { type: null, tname: 'default', item: { client: 5, clock: 2 }, assoc: 0 } };
    const out = screenPresence([entry(5, { user: { name: '관리자', color: 'red' }, cursor })], U, new Map([[5, U]]), '진짜 이름');
    expect(out).toEqual({ keep: [entry(5, { user: { name: '진짜 이름', color: 'red' }, cursor })], bind: [] });
  });
  ...
  it('같은 프레임에 섞여 와도 항목마다 따로 판정한다', () => {
    const out = screenPresence([entry(5, { user: { name: 'a' } }), entry(6, { user: { name: 'b' } }), entry(8, null)], U, new Map([[5, U], [6, X]]), 'U');
    expect(out.keep.map((e) => e.client)).toEqual([5, 8]);
    expect(out.bind).toEqual([8]);
  });
});
