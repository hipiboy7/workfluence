import { CSRF_HEADER, CSRF_HEADER_VALUE, createLineSplitter, parseLlmEvent, type LlmAskDto, type LlmStreamEvent } from '@workfluence/shared';
 ...
 * `api()`와 같게 같은 출처 쿠키와 CSRF 머리말을 붙인다(7절).
 */
export async function askLlm(dto: LlmAskDto, onEvent: (e: LlmStreamEvent) => void, fetchImpl: typeof fetch = fetch): Promise<void> {
  const res = await fetchImpl('/api/llm/ask', {
    method: 'POST',
...
export type ChatState = {
  /** `idle` 묻기 전·끝난 뒤 · `streaming` 받는 중 · `stopping` 멈추라고 했고 끝을 기다린다 */
  phase: 'idle' | 'streaming' | 'stopping';
...
export function chatReducer(state: ChatState, action: ChatAction): ChatState {
  switch (action.type) {
    case 'send':
      return { phase: 'streaming', live: { question: action.question, answer: '', thinking: '' }, error: null, notice: null, restore: null, lastEnd: null };
    case 'event': {
