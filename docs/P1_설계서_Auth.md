...
- 작성일: 2026-09-21 / 작성 LLM: Claude Opus 5
...
- 계정 복구: ID 찾기(마스킹), 비밀번호 초기화(임시 비밀번호 1회 표시)
...
| FR-209 | 비밀번호 초기화는 임시 비밀번호를 **응답에 1회만** 싣고 `must_change_password`를 켠다. 저장하지 않는다 | `CLAUDE.md` 7절 |
...
| FR-224 | 로그아웃·비밀번호 변경 시 **서버측 세션을 파기**한다 | `CLAUDE.md` 7절 |
...
| `/signup` | 가입 요청 | 공개 |
| `/find-id`, `/recover-password` | 계정 찾기 | 공개 |
| `/change-password` | 비밀번호 변경 | 로그인 |
| `/admin/users` | 사용자 관리 | `user.manage` |
| `/admin/audit` | 감사로그 | `audit.read` |

임시 비밀번호는 `CopyableSecret` 컴포넌트로 **1회만** 보여 준다 (프로토타입에 있다). 다시 볼 수 없다는 것을 화면에 적는다.
