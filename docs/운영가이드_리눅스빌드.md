...
```bash
echo "WF_PG_PASSWORD=$(openssl rand -hex 16)" > deploy/.env
```

**한 줄만 필요한 이유.** 앱이 쓰는 환경변수는 `deploy/compose.yml`이 직접 넘긴다. 이 파일에서 읽는 것은 데이터베이스 비밀번호 하나뿐이고, 그것만 사람이 정해야 하는 값이다. 나머지는 기본값이 있다.
...
```bash
start=$(date +%s)
docker compose -f deploy/compose.yml --env-file deploy/.env up -d postgres api
...
| 증상 | 원인 | 조치 |
|---|---|---|
| `WF_PG_PASSWORD를 .env에 설정` | 환경 파일이 없거나 비었다 | 4절을 다시 한다. `--env-file deploy/.env`를 빼먹지 않았는지, 루트 `.env`를 가리키고 있지 않은지 확인한다 |
...
| 기동하다 바로 죽는다 | 설정 항목 오류 | 로그 첫 줄을 본다. 모르는 설정 키가 있으면 **일부러 기동을 멈춘다**. 개발용 `.env`를 `--env-file`로 넘기면 이 증상이 난다 (4절) |
