...
### 2.5 관문이 목록 항목을 적용해 보는 비용 (설계서 A.1-3)

목록 구조를 건드리는 변경만 서버 문서를 복제해 적용한다. 문단 N개와 목록 항목 50개가 든 문서에서 변경 하나를 판정하는 데 걸린 시간 (`judge`, 한 번씩 잰 값 — 판정은 모두 받음):
...
글자 치기는 흉내에 닿지 않는다. 가장 큰 문서에서 목록 구조를 바꾸는 한 번이 40ms다.
...
자체 점검 5건(넷은 코드 리뷰와 겹침), 오탐 0. 가장 큰 것은 보류 25를 닫은 근거의 정정이다(2.1 정정, 2.5).
