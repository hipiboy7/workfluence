...
| 대상 | 건수 | 커버리지 |
|---|---|---|
| `packages/shared` | 58 (document 11 · env 14 · permissions 15 · schemas 11 · security 7) | — |
| `apps/api` | **93** (Phase 0 18 + 신규 75) | **문장 89.4% · 브랜치 85.9% · 함수 85.1% · 라인 93.1%** |
| `apps/api/src/auth/domain` (A등급) | 20 | 임계 90% 통과 |
| E2E (`e2e/`) | **9** | 측정 제외 |
| skip | **0** | |

신규 75건의 내역: A등급 순수 함수 20(claims 10 · lockout 10 — **테스트를 먼저 썼다**),
가드 14, 감사 마스킹 4, 모의 제공자 5, **실제 PostgreSQL 통합 32**.

> **앞 판의 수치가 틀렸다.** "A등급 29(lockout 19)"로 적었는데 실제 `lockout.spec.ts`는 10건이고
> 합계도 맞지 않았다. 자체 점검이 잡았다. 검증기록에서 수치가 틀리면 그 문서 전체를 못 믿는다 —
> 세어서 적는다.
...
| `apps/api/src/**/*.module.ts` | 컨트롤러가 하는 일은 트랜잭션 안에서 서비스와 감사를 부르는 배선이고 그 둘은 각각 측정된다. **배선이 맞는지는 E2E가 본다** — 그래서 E2E가 없으면 이 제외가 곧 구멍이다. 자체 점검이 그 구멍을 지적해(#6) OIDC 로그인·승인·역할 변경·초기화·감사로그 조회를 E2E에 추가했다 |
...
| NFR-16 | 보안 점검 | `/security-review` 통과 | 높음 1·보통 3 지적 → 전부 반영 (검토서 참고) | — |
