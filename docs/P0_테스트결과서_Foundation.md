...
만들자마자 자기 저장소에서 위반을 찾아냈다. 내역: 존재하지 않는 파일 참조 5건(`docs/배포가이드.md` 등 Phase 5 산출물), 자리표시자 `pnpm <script>`를 명령으로 오인 4건, 아직 없는 Phase 3 스크립트 1건, 범위 표기(`prototype-v1~v3.md`) 1건, 이번 Phase에서 만들 문서 3건.
...
> nginx는 인증서(`deploy/certs/cert.pem`·`key.pem`)가 있어야 뜬다. Phase 0 검증에서는 `postgres`·`api`만 올려 확인하고, TLS 종단은 Phase 5 배포가이드에서 다룬다.
