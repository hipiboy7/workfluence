...
바꾼 뒤에는 **관문이 여전히 잡는지 자가시험**했다. 문서에 없는 경로와 대소문자가 틀린 경로를 각각 한 줄씩 넣어 둘 다 위반으로 보고되는 것을 확인하고 되돌렸다. 통과만 확인하면 "아무것도 검사하지 않는 관문"과 구별되지 않는다. 이 시험에서 접두 비교가 대소문자를 구분해 `Docs/`를 경로로 보지 않고 흘려보내던 것도 드러나 함께 고쳤다.
