...
바꾼 뒤에는 **관문이 여전히 잡는지 자가시험**했다. 문서에 없는 경로와 대소문자가 틀린 경로를 각각 한 줄씩 넣어 둘 다 위반으로 보고되는 것을 확인하고 되돌렸다. 통과만 확인하면 "아무것도 검사하지 않는 관문"과 구별되지 않는다. 이 시험에서 접두 비교가 대소문자를 구분하는 탓에 첫 글자가 대문자인 경로를 "경로가 아니다"로 흘려보내던 것도 드러나 함께 고쳤다.

### 6.9 문서 재독에서 CI 서술이 옛 상태로 남아 있었다

Phase 종료 루틴(`CLAUDE.md` 1.1절 8-1)의 `doc-consistency` 재독이 두 곳을 잡았다. CI를 단계별로 쪼갠 변경이 워크플로와 이 문서에는 반영됐는데, **설계서 9.2절의 CI 표**와 **요구사항정의서 FR-091**은 "`pnpm check` 한 덩어리"로 남아 있었다. 취약점 점검 명령에 붙은 `--prod`도 표에 없었다.

부분 치환이 놓치는 전형이다 — 바꾼 사람은 자기가 만진 파일만 본다. 두 곳 모두 실제 워크플로에 맞춰 고치고, 왜 한 줄로 묶지 않는지와 `--prod`를 붙이는 이유를 설계서에 적었다. 같은 재독에서 **비밀번호 정책·실측 수치·설정 키 12개**는 네 곳이 서로 일치함을 확인했다.
