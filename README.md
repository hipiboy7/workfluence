...
**현재 단계**: Phase 11까지 완료 (운영 로그·LLM 관리 위임, 2026-09-26 — 요청마다 번호가 붙어 nginx 로그·앱 로그·감사로그를 한 번호로 잇고, 시스템 관리자가 관리자 한 사람씩 LLM 연결 관리를 맡긴다. 사내 LLM은 반입 뒤 현장에서 연결한다). Phase 10에서 사내 LLM 질문(시스템 관리자가 등록한 LLM에 메뉴에서 묻고, 대화는 기간·개수만큼 남는다)이 들어왔다. Phase 6에서 실시간 동시 편집·버전 비교·HTML 내보내기·템플릿·멘션 메일이 들어왔고, PDF 내보내기와 Confluence 가져오기는 하지 않기로 닫았다.
